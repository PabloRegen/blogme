...
module.exports = function(knex) {
	return function(postId, tagsToKeep) {
		return Promise.try(() => {
			...
			/* remove tags_posts associations for all tags removed by the user from this post */
			return knex('tags_posts').delete().whereNotIn('tagId', tagIds).where({
				postId: postId
			}).returning('tagId');
		}).then((removedTagIds) => {
			/* for each tag for which a tag association was removed, soft-delete the tag if there are no other associations for it */
			return knex('tags').whereIn('id', removedTagIds).update({
