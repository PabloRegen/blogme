			...
			return Promise.try(() => {
				...
				return knex.transaction(function(trx1) {
					return Promise.try(() => {
						...
						function removeTag(diffRemoved) {
							...
							return Promise.map(diffRemoved, (removedTag) => {
								return Promise.try(() => {
						...
						function storeTag(diffAdded) {
							...
							return Promise.map(diffAdded, (addedTag) => {
								return Promise.try(() => {
						...
						return Promise.try(() => {
							return removeTag(diff.removed);
						}).then(() => {
							return storeTag(diff.added);
						});
					});
				});
