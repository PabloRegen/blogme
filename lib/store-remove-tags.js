...
function removeTag(postId, tag, trx1) {
	...
	return Promise.try(() => {
		return trx1('tags').where({name: tag.name}).first();
	}).then((result) => {
		let tagId = result.id;
		...
		return Promise.try(() => {
			...
			if (deletedAssociations.length === 0) {
				...
				throw new errors.AlreadyDeletedError(`There was no tag_post association to delete for the removed "${tag.name}" tag`);
			} else {
			...
			if (additionalTagPostAssociation == null) {
				/* case a */
				return trx1('tags').update({deletedAt: trx1.fn.now()}).where({id: tagId});
			}
		});
	});
}

function storeTag(postId, tag, trx1) {
	...
	return Promise.try(() => {
		...
		return trx1.transaction(function(trx2) {
			return trx2('tags').insert({name: tag.name}).returning('id');
		});
	}).catch(databaseError.rethrow).catch(duplicateTag, (err) => {
		/* case b */
		return trx1('tags').update({deletedAt: null}).where({name: tag.name}).returning('id');
	}).then((tagIds) => {
...
module.exports = function(knex) {
	return function(postId, newTags) {
		...
		function storeRemoveTags() {
					...
					return Promise.try(() => {
						...
						return Promise.try(() => {
							return Promise.map(diff.removed, (removedTag) => {
								return removeTag(postId, removedTag, trx1);
							...
							return Promise.map(diff.added, (addedTag) => {
								return storeTag(postId, addedTag, trx1);
			...
			}).catch(errors.AlreadyDeletedError, duplicateAssociation, (err) => {
